...
export default function Admin() {
  ...
  useEffect(() => {
    ChatbotConfig.list().then(configs => {
      ...
      else setConfig({ bot_name: "Avicam Assistant", is_active: true, model: "gpt-4o-mini", temperature: 0.7, max_tokens: 500, system_prompt: "", welcome_message: "", suggested_questions: "", custom_qa: "" });
    });
    loadInquiries();
  }, []);
  ...
  const loadInquiries = () => {
    ...
    ContactInquiry.list({ sort: "-created_date" }).then(data => {
      setInquiries(data || []);
        ...
        <div className="flex gap-1 mb-6 border-b border-[#c9a84c]/10">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-5 py-3 text-xs tracking-[0.15em] transition-colors border-b-2 -mb-px ${tab === t.id ? "text-[#c9a84c] border-[#c9a84c]" : "text-gray-500 border-transparent hover:text-gray-300"}`}>
              {t.label.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        {tab === "inquiries" && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-2 space-y-2">
              ...
              {loadingInquiries ? (
                <div className="text-center py-10 text-gray-600 text-sm animate-pulse">Loading...</div>
              ) : inquiries.length === 0 ? (
                ...
                inquiries.map(inq => (
                  <div key={inq.id} onClick={() => setSelectedInquiry(inq)}
                    className={`p-4 border cursor-pointer transition-all ${selectedInquiry?.id === inq.id ? "border-[#c9a84c]/50 bg-[#c9a84c]/5" : "border-[#c9a84c]/10 bg-[#0d0d0d] hover:border-[#c9a84c]/30"}`}>
              ...
              {!selectedInquiry ? (
                ...
                <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-6 space-y-5">
                  <div className="flex justify-between items-start">
                    <div>
                      <h2 className="text-xl font-light text-white">{selectedInquiry.name}</h2>
                      <p className="text-gray-500 text-xs mt-0.5">{new Date(selectedInquiry.created_date).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}</p>
                    </div>
                    <div className="flex gap-2">
                      {["New","In Review","Responded"].map(s => (
                        <button key={s} onClick={() => updateStatus(selectedInquiry.id, s)}
                          className={`text-xs px-3 py-1 border transition-colors ${selectedInquiry.status === s ? STATUS_COLORS[s] : "border-gray-700 text-gray-500 hover:border-gray-500"}`}>
                          {s}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t border-[#c9a84c]/10 pt-4">
                    {[
                      ...
                      ["Event Type", selectedInquiry.event_type || "—"],
                      ["Destination", selectedInquiry.destination || "—"],
                      ["Date", selectedInquiry.date || "—"],
                      ["Guests", selectedInquiry.guests || "—"],
                    ].map(([label, val]) => (
                      <div key={label}>
                        <p className="text-xs tracking-[0.25em] text-gray-600 mb-1">{label.toUpperCase()}</p>
                        <p className="text-gray-300 text-sm font-light">{val}</p>
                      </div>
                    ))}
                  </div>
                  {selectedInquiry.message && (
                    <div className="border-t border-[#c9a84c]/10 pt-4">
                      <p className="text-xs tracking-[0.25em] text-gray-600 mb-2">MESSAGE</p>
                      <p className="text-gray-300 text-sm font-light leading-relaxed">{selectedInquiry.message}</p>
                    </div>
                  )}
                  <div className="border-t border-[#c9a84c]/10 pt-4 flex gap-3">
                    <a href={`mailto:${selectedInquiry.email}`}
                      className="px-5 py-2 bg-[#c9a84c] text-black text-xs tracking-[0.2em] hover:bg-[#e0c070] transition-colors font-semibold">
                      REPLY BY EMAIL
                    </a>
                    {selectedInquiry.phone && (
                      <a href={`https://wa.me/${selectedInquiry.phone.replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer"
                        className="px-5 py-2 border border-[#c9a84c]/40 text-[#c9a84c] text-xs tracking-[0.2em] hover:bg-[#c9a84c]/10 transition-colors">
                        WHATSAPP
                      </a>
                    )}
                  </div>
        ...
        {tab === "general" && config && (
          <div className="space-y-5 max-w-3xl">
            <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-5 flex items-center justify-between">
              <div>
                <h3 className="text-white font-light text-sm mb-0.5">Chatbot Active</h3>
                <p className="text-gray-500 text-xs">Enable or disable the chat widget</p>
              </div>
            ...
            {[
              { label: "BOT NAME", field: "bot_name", type: "input", placeholder: "e.g. Avicam Assistant" },
            ].map(({ label, field, placeholder }) => (
              <div key={field} className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-5">
                <label className="block text-xs tracking-[0.3em] text-gray-500 mb-3">{label}</label>
                <input value={config[field] || ""} onChange={e => set(field, e.target.value)}
            ...
            <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-5">
              ...
              <p className="text-gray-600 text-xs mb-3">One per line. Max 4 shown as quick buttons.</p>
              <textarea value={config.suggested_questions || ""} onChange={e => set("suggested_questions", e.target.value)} rows={5}
          ...
          <div className="space-y-5 max-w-3xl">
            <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-6">
              ...
              <div className="bg-[#111] border border-[#c9a84c]/10 px-4 py-3 mb-4 text-xs text-gray-400 font-mono leading-relaxed">
                ...
                A: Yes, we produce events across Israel.
              </div>
              <textarea value={config.custom_qa || ""} onChange={e => set("custom_qa", e.target.value)} rows={16}
                className="w-full bg-[#111] border border-[#c9a84c]/20 text-white px-4 py-3 focus:outline-none focus:border-[#c9a84c] transition-colors font-light resize-none text-sm leading-relaxed"
          ...
          <div className="space-y-5 max-w-md">
            <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-5">
              ...
              <select value={config.model || "gpt-4o-mini"} onChange={e => set("model", e.target.value)}
                className="w-full bg-[#111] border border-[#c9a84c]/20 text-white px-4 py-3 focus:outline-none focus:border-[#c9a84c] font-light text-sm">
        ...
        {tab === "test" && (
          <div className="max-w-2xl space-y-5">
            <div className="bg-[#0d0d0d] border border-[#c9a84c]/15 p-6">
              ...
              <textarea value={testMsg} onChange={e => setTestMsg(e.target.value)} rows={4} placeholder="Ask the bot something..."
                className="w-full bg-[#111] border border-[#c9a84c]/20 text-white px-4 py-3 focus:outline-none focus:border-[#c9a84c] transition-colors font-light resize-none text-sm mb-4" />
