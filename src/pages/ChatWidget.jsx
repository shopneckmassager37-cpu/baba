    ...
    <>
      {showBubble && !open && (
        <div onClick={() => setOpen(true)} className="fixed bottom-24 right-6 z-[9997] cursor-pointer" style={{ animation: "fadeInUp 0.4s ease" }}>
          <div className="bg-white text-gray-800 px-4 py-3 shadow-2xl max-w-[210px] text-sm font-light leading-relaxed relative" style={{ borderRadius: "12px 12px 0 12px", fontFamily: "'Georgia', serif" }}>
        ...
        <div className="fixed bottom-24 right-6 z-[9998] w-[360px] flex flex-col shadow-2xl overflow-hidden"
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3" style={{ maxHeight: "360px" }}>
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[80%] px-4 py-3 text-sm font-light leading-relaxed whitespace-pre-line ${m.role === "user" ? "bg-[#c9a84c] text-black" : "bg-[#1d1d1d] text-gray-200 border border-[#c9a84c]/10"}`}
          ...
          {suggested.length > 0 && (
            <div className="px-4 pb-3 flex flex-wrap gap-2 border-t border-[#c9a84c]/10 pt-3">
              {suggested.map((q, i) => (
                <button key={i} onClick={() => send(q)} className="text-xs text-[#c9a84c] border border-[#c9a84c]/30 px-3 py-1.5 hover:bg-[#c9a84c]/10 transition-colors font-light text-left" style={{ borderRadius: "20px" }}>{q}</button>
              ))}
            </div>
          )}
          ...
          <div className="border-t border-[#c9a84c]/15 p-3 flex gap-2" style={{ background: "#0d0d0d" }}>
            ...
            <button onClick={() => send()} disabled={loading || !input.trim()}
              className="bg-[#c9a84c] text-black px-4 py-2 hover:bg-[#e0c070] transition-colors disabled:opacity-40" style={{ borderRadius: "8px" }}>
